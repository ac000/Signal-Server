...
int min_north = 90, max_north = -90, min_west = 360, max_west = -1, ippd, mpi,
    max_elevation = -32768, min_elevation =
    32768, bzerror, contour_threshold, output = 2;
		 ...
		 int pngwidth, int pngheight)
{
	...
	fprintf(fd, "set xrange [-0.5 to %2.3f]\n",
		metric ? KM_PER_MILE * rint(distance + 0.5) : rint(distance +
								   0.5));
	...
	if ((LR.frq_mhz >= 20.0) && (LR.frq_mhz <= 100000.0) && fresnel_plot)
		...
		fprintf(fd,
			"set title Height Profile Between %s and %s (%.2f%c)\"\n",
			source.name, destination.name, azimuth, 176);

	if (metric)
		fprintf(fd,
			"set xlabel \"Distance: %.2f Km Path Loss: %.2f dB Received Power: %.2f dBm\"\n",
			KM_PER_MILE * Distance(source, destination), loss, dBm);
	else
		fprintf(fd,
			"set xlabel \"Distance: %.2f Mi Path Loss: %.2f dB Received Power: %.2f dBm\"\n",
	...
	if ((LR.frq_mhz >= 20.0) && (LR.frq_mhz <= 100000.0) && fresnel_plot) {
		if (clutter > 0.0) {
			...
			fprintf(fd,
				"plot \"profile.gp\" title \"Terrain\" with filledcurve x1 lt 1 lc rgb \"#cbbc8a\", \"reference.gp\" title \"Line of sight\" with lines, \"curvature.gp\" axes x1y2 title \"Earth's Curvature\" with lines, \"fresnel.gp\" axes x1y1 title \"Fresnel Zone for %.1f MHz\" with lines, \"fresnel_pt_6.gp\" title \"%.0f%% of Fresnel Zone\" with lines\n",
...
*/
	...
	if (graph_it) {
		...
		fprintf(fd,
			"set title \"%s Loss Profile Along Path Between %s and %s (%.2f%c azimuth)\"\n",
			ppa_name, destination.name, source.name,
...
void SiteReport(struct site xmtr)
{
	...
	if (terrain > -4999.0) {
		if (metric)
			fprintf(fd,
				"Antenna height above average terrain: %.2f meters\n\n",
				METERS_PER_FOOT * terrain);
		else
			fprintf(fd,
				"Antenna height above average terrain: %.2f feet\n\n",
				terrain);
		...
		for (azi = 0; azi <= 315; azi += 45) {
			fprintf(fd, "Average terrain at %3d degrees azimuth: ",
				azi);
...
int main(int argc, char *argv[])
{
	int x, y, z = 0, min_lat, min_lon, max_lat, max_lon,
	    rxlat, rxlon, txlat, txlon, width = 1000, height = 350, winfiles =
	    0;
	...
	double er_mult, txla = 999, txlo = 999, rxla = 999, rxlo = 999, txh =
	    0, rxh = 0;
	...
	if (argc == 1) {
		...
		fprintf(stdout,
			"       -wf Windows SDF tiles with equals not colons\n");
...
// 100 mile limit
	...
	GraphHeight(tx_site[0], rx_site, string, fresnel_plot, norm, width,
		    height);
	...
	GraphHeight(rx_site, tx_site[0], string, fresnel_plot, norm, width,
		    height);
